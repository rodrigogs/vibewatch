...
fn test_watcher_detects_file_creation() {
    ...
    assert!(marker_file.path().exists(), 
...
fn test_filter_include_pattern_only_watches_matching_files() {
    ...
    assert!(counter_file.path().exists(), 
...
fn test_specific_event_commands() {
    ...
    assert!(create_marker.path().exists(), "Create command was not executed");
    assert!(modify_marker.path().exists(), "Modify command was not executed");
    assert!(delete_marker.path().exists(), "Delete command was not executed");
}
